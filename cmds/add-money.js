...
module.exports.run = async (bot,message,args) => {
    ...
    let em2 = message.guild.emojis.find(emoji => emoji.name === "animated_cross")
    if (!message.member.hasPermission('MANAGE_GUILD')) { // if message.author / member does not have the permission MANAGE_GUILD, return.
        return message.channels.send(`${em2} Вы не можете использовать эту команду!`).then(msg => {
            setTimeout(() => {
                msg.delete()
    ...
    if (isNaN(args[0])) return message.channel.send(`${em2} ${message.author}, вам нужно ввести допустимое число для прибавления.`) // if args[0] (first input) is not a number, return.
    db.add(`coins_${argsUser.id}`, args[0])
    let bal = await db.fetch(`coins_${argsUser.id}`)

    let embed = new Discord.RichEmbed()
    .setAuthor(`${em1} Добавлены 🌞 монеты!`)
